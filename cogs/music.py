...
class Music(commands.Cog):
    ...
    async def help(self, ctx):
      ...
      embed.add_field(name="Undefined Commands", value="`jumpqueue`", inline=False)
      embed.set_footer(text="This bot was developed by Yuan Mizuna#9666 and The Starking Developers Group. Private bot.")
    ...
    def _play_song(self, client, state, song):
        ...
        if state.is_looping is True:
         client.play(source, after=repeat)
        else:
    ...
    async def loop(self, ctx):
      state = self.get_state(ctx.guild)
      if state.is_looping is False:
        state.is_looping = True
        await ctx.send("Now Looping the song.")
      else:
       state.is_looping = False
       await ctx.send("Loop are now disabled.")
